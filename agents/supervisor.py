...
def supervisor_node(state: AgentState) -> dict:
    ...
    try:
        ...
        def _ingest(source, category_name):
            ...
            Attempts to parse each string item as JSON (after removing any agent header).
            If parsing fails, falls back to splitting the string into bullet points.

            Args:
                source (list): List of findings, each as a string (agent output).
            ...
            for item in source:
                if isinstance(item, str):
                    # Try to extract JSON findings from the string
                    try:
                        # Remove header if present
                        if item.startswith(
                            "**Generated by Multi-Agent PR Review System"
                        ):
                            _, json_part = item.split("\n\n", 1)
                        else:
                            json_part = item
                        ...
                        # Fallback: treat as plain text/bullets
